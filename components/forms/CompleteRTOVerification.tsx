...
const ConfirmedRto = ({leadId = 'new'}: FormComponentProps): JSX.Element => {
  ...
  const manufacturingYear =
    ...
    format(Date.parse(leadInput?.vehicle?.tempManufacturingDate), 'yyyy')
  // FIXME : change the variable names to temp , wherever deemed necessary
  const engineNumber = leadInput?.vehicle?.tempEngineNumber
  const chassisNumber = leadInput?.vehicle?.tempChassisNumber
  const tempFinancerName =
  ...
  useEffect(() => {
    ...
    // When user comes from lead update, this takes care of edge case when there is no document in second case.
    let currentLead = Object.assign({}, leadInput) //create a deep copy of lead input
    const approvalMailUrl =
      currentLead?.vehicle?.documents?.approvalMailUrl ?? ''
    const rtoVerificationProofUrl =
      currentLead?.vehicle?.documents?.rtoVerificationProofUrl ?? ''
    const hsrbProofUrl = currentLead?.vehicle?.documents?.hsrbProofUrl ?? ''
    const challanUrl = currentLead?.vehicle?.documents?.challanUrl ?? ''
    const hypothecationProofUrl =
      currentLead?.vehicle?.documents?.hypothecationProofUrl ?? ''
    // FIXME : there could be validation bug
    currentLead = {
      ...currentLead,
      vehicle: {
        ...leadInput?.vehicle,
  ...
  // console.log(manufacturingYearList)
  return (
